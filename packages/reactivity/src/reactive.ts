...
function getTargetType(value: Target) {
  // value['__v_skip'] 内部标识符 标识是否跳过
  // !Object.isExtensible(value) 返回一个值，该值指示是否可以向对象添加新属性。
 ...
 * @param baseHandlers 基本类型的处理器
 * @param collectionHandlers 集合类型的处理器
 * @param proxyMap 代理对象的缓存Map
 */
function createReactiveObject(
  ...
  // 检查目标对象是否已经是代理对象,如果是则直接返回原本值
  // target['__v_raw'] 内部标识符
  // isReadonly 是函数参数传递过来的值, 标识是否只读
  // target['__v_isReactive'] 内部标识符 标识是否是响应式对象
  // 如果 target['__v_raw'] 存在，并且 isReadonly 为 false 或者 target['__v_isReactive'] 存在，则直接返回原本值
  if (
